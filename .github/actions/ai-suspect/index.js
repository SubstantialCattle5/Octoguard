module.exports = async ({ github, context }) => {
  ...
  const bodyHints = [
    /\bas an ai\b/i, /\bgenerated by\b/i, /\bcopilot\b/i, /\bchatgpt\b/i,
    /\bgpt-[0-9]/i, /\bclaude\b/i, /\bllm\b/i, /\bopenai\b/i,
    /\bai assistant\b/i, /\bai-generated\b/i
  ];
  ...
  if (massiveChange) indicators.push(`📊 Very large diff (${totalChanges} changes) - unusual for human PR`);
  
  for (const f of files) {
    const patch = f.patch || "";
    if (/\.(md|txt|rst)$/i.test(f.filename) || f.filename.startsWith('.github/')) continue;
    if (aiMarkers.test(patch)) indicators.push(`📝 AI marker found in ${f.filename}`);
    if (aiToolMentions.test(patch)) indicators.push(`🤖 AI tool mentioned in ${f.filename}`);
    const addedLines = patch.split("\n").filter(l => l.startsWith("+") && !l.startsWith("+++"));
    const commentAdds = addedLines.filter(l => /^(\+\s*(\/\/|#|\/\*\*?|\*|"""|'''))/.test(l)).length;
  ...
  if (label) {
    ...
    const message = aiDisclosed
      ? `### ✅ AI Assistance Disclosed\n\nThis PR has been labeled \`${label}\` and will require human review.\n\n${summary}`
      : `### 🤖 Potential AI-Generated Content Detected\n\nThis PR has been labeled \`${label}\` based on these indicators:\n\n${summary}\n\nIf this is incorrect, please clarify in the PR description.`;
